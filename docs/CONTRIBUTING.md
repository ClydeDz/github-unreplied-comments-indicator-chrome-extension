...
- Once installed, if you open up a GitHub pull request, spot the comment(s) that have no replies. They should be highlighted with a theme from this extension.
- You can select which theme you want by going to the options page of this extension.
...
- Update the version number in the [manifest.json file](https://github.com/ClydeDz/github-unreplied-comments-indicator-chrome-extension/blob/main/src/manifest.json#L5) and [package.json file](https://github.com/ClydeDz/github-unreplied-comments-indicator-chrome-extension/blob/main/package.json#L3). We're following the [Semver guide](https://semver.org/), so please check the website to see which number you'd need to increment.
